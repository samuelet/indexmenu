...
$lang['page_index']            = '주 도쿠위키 색인을 바꿀 문서입니다. 문서를 만들고 Indexmenu 문법을 넣으세요. navbar 옵션으로 Indexmenu 사이드바가 이미 있으면 <code>id#random</code>을 사용하세요. 내 제안은 <code>{{indexmenu>..|js navbar nocookie id#random}}</code>입니다.';
$lang['empty_msg']             = '트리가 비어있을 때 보여줄 메시지입니다. 도쿠위키 문법을 사용하고 html 코드를 사용하지 마세요. <code>{{ns}}</code> 변수는 요청한 이름공간에 대한 단축입니다.';
$lang['skip_index']            = '건너뛰는 이름공간 id입니다. 정규 표현식 형식을 사용하세요. 예: <code>/(사이트바|비공개:내이름공간)/</code>';
$lang['skip_file']             = '건너뛰는 문서 id입니다. 정규 표현식 형식을 사용하세요. 예: <code>/(:start$|^public:newstart$)/</code>';
